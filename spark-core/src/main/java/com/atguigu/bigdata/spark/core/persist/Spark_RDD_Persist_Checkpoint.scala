...
object Spark_RDD_Persist_Checkpoint {
...
//    tupleRDD.persist(StorageLevel.MEMORY_AND_DISK)
    /**
     * 保存到 setCheckpointDir 的路径，非临时文件，程序执行完文件也还在;
     * 但checkpoint每次运行点要重新执行一遍;一般是和cache()配合使用以提高效率，
     */
//    tupleRDD.cache()  //不加cache(),'@@@@@'输出12次; 反之输出6次
    tupleRDD.checkpoint()
